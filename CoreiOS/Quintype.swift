...
open class Quintype{
    ...
    open static func initWithBaseUrl(baseURL: String!) {
        Quintype.sharedInstance._baseURL = baseURL
    ...
    open static var analytics:Analytics{
        ...
        get{
            let defaults = UserDefaults.standard
            if defaults.hasKey(Constants.publisherConfig.publisherKey){
                if Quintype.sharedInstance._analytics == nil{
                    Quintype.sharedInstance._analytics = Analytics()
                }
                return Quintype.sharedInstance._analytics!
            }else{
                Quintype.sharedInstance.LoadConfig(success: {
                    if Quintype.sharedInstance._analytics == nil{
                        Quintype.sharedInstance._analytics = Analytics()
                    }
                })
    ...
    private func LoadConfig(success:@escaping ()->()){
        
        
        
        if let _ = Quintype.sharedInstance._baseURL{
            
            Cache.retriveCacheData(keyName: Constants.publisherConfig.publisherCacheKey) { (data) in
                //print(data as Any)
                
                if data == nil{
                    //TODO: - ADD 10 sec delay
                    print("failled.....10 sec delay")
                    Quintype.sharedInstance.LoadConfig(success: {})
                }else{
                    ApiParser.configParser(data: data as! [String : AnyObject]?, completion: { (data) in
            ...
            print("no base URL.....10 sec delay")
             Quintype.sharedInstance.LoadConfig(success: {})
        }
